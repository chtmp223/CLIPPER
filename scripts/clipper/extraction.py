...
def multiple_init(outlines): 
    ...
    for i, out in enumerate(outlines): 
        event = f"## Chapter {i+1} outline\n" + extract_tag_text(out, "events", random=False).strip()
        all_events.append(event)
    event_text = "\n\n".join(all_events)
    
    to_prompt = open("../../prompts/extraction_multiple.md", "r").read()
    prompt = to_prompt.format(chapter_outlines=event_text)
    responses = prompt_claude(prompt, verbose=True, metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0}, system_message="You are an expert at extracting facts from fictional narratives.")

    # Processing the responses
    if "<facts>" not in responses:
        print(responses)
        return pd.DataFrame({"facts": [], "explanation": [], "source": [], "brainstorm": [], "corrupted": [], "corrupted_reasoning": []})
    if "</facts>" not in responses: 
        responses += "</facts>"
        ...
        try: 
            ...
            if f"</fact_{i+1}>" not in claims: 
                if f"<fact_{i+2}>" in claims: 
                    claims = claims.replace(f"<fact_{i+2}>", f"</fact_{i+1}>\n\n<fact_{i+2}>")
                else: 
                    claims += f"</fact_{i+1}>"
    ...
    df = pd.DataFrame({"facts": claim_text, "corrupted_facts": corrupted, "fact_reasoning": claim_exp, "corrupted_reasoning": corrupted_reasoning, "source": source, "brainstorm": brainstorms})
...
def multiple_cont(existing_df, outlines, times=10): 
    ...
    for time in trange(times): 
        ...
        existing_claims = "\n".join([f"- Fact {i+1}: {fact}" for i, fact in enumerate(existing)])
            
        prompt = prompt.format(chapter_outlines=event_text, existing_facts=existing_claims)
        responses = prompt_claude(prompt, verbose=False, metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0}, system_message="You are an expert at extracting facts from fictional narratives.")
        if "<facts>" not in responses:
            print(responses)
            return pd.DataFrame({"facts": [], "explanation": [], "source": [], "brainstorm": [], "corrupted": [], "corrupted_reasoning": []})
        if "</facts>" not in responses: 
...
def single(outlines, existing_df, book_name): 
    ...
    for i, chap in enumerate(tqdm(outlines)): 
        existing = "\n".join([f"- Fact {i+1}: {fact}" for i, fact in enumerate(existing_facts_list)])
        prompt = p.format(book_summary=summary, chapter_outline=f"## Chapter {i+1} outline\n" + extract_tag_text(chap, "events", random=False).strip(), existing_facts=existing)
        response = prompt_claude(prompt, verbose=False, metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0}, system_message="You are an expert at extracting facts from fictional narratives.")
...
if __name__ == "__main__": 
    ...
    parser.add_argument("--iteration", help="Number of times to continue extracting claims from multiple chapters", default=10)
