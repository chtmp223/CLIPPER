...
def get_chapters(book_name):
    ...
    if book_name in os.listdir(f"../../data/books/"): 
        folder = [f for f in os.listdir(f"../../data/books/{book_name}") if f.endswith(".txt") and f != "full.txt"]
    ...
    for i in trange(len(chapters)): 
        ...
        while retry < 3:        # Retry if the formatting is not correct
            try: 
                ...
                outline = prompt_claude(prompt, verbose=False, metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0}, system_message="You are an expert at creating objective outlines of fictional narratives.")
                if "<synopsis>" in outline and "<events>" in outline and "<characters>" in outline: 
                    outline = "<synopsis>" + outline.split("<synopsis>")[1]
                    outline = outline.split("</characters>")[0] + "</characters>"
                    previous_chapters.append(outline)
                    break
                else:   
                    print(f"Wrong outline format for chapter {i}. Retrying #{retry+1}...")
            except Exception as e:
                traceback.print_exc()
            retry += 1

        if not ("<synopsis>" in outline and "<events>" in outline and "<characters>" in outline):
...
def post_processing(outlines): 
    ...
    for i, out in enumerate(outlines): 
        try: 
            if "</events>" not in out or "</characters>" not in out or "</synopsis>" not in out:
            ...
            previous_text = [clean_word(word.lower()) for word in previous_text.split(" ")]
            ...
            for char in characters:
                ...
                if " " in name: 
                    ...
                    if len(name) > 0 and adding == True:
                        char = name.strip("[]") + ": " + ":".join(char.split(":")[1:]).strip()
            ...
            new_characters = "\n".join([f"{i+1}. {char}" for i, char in enumerate(final_characters)])
...
def gen_character_status(outlines): 
    ...
    for i, outline in enumerate(outlines): 
        ...
        character = [line for line in extract_tag_text(outline, "characters").split('\n') if len(line)>0]
        chars = [". ".join(char.strip().split(":")[0].split(". ")[1:]) for char in character]
        splitted = [clean_word(n.lower()) for name in chars for n in name.split(" ") if clean_word(n.lower()) not in FILLER and len(clean_word(n.lower())) > 0]
        affliation = []
            ...
            if " " in name: 
                ...
                for n in names: 
                    ...
                    if (n in characters and n not in FILLER): 
                        adding = True
                if len(name) > 0 and adding == True:
                    char = character[j].split(": ")[0] + " (existing): " + ": ".join(character[j].split(": ")[1:])
                    affliation.append(char)
                    character_status['existing'].append(name) 
                else: 
                    char = character[j].split(": ")[0] + " (new): " + ": ".join(character[j].split(": ")[1:])
                    affliation.append(char)
        ...
        out.append(outlines[i].replace(extract_tag_text(outlines[i], "characters"), "</characters>" + "\n".join(affliation) + "</characters>")).replace("<characters><characters>", "<characters>").replace("</characters></characters>", "</characters>")
...
def outline_gen_main(book_name): 
    ...
    assert len(outlines) == len(chapters), "Number of outlines generated does not match the number of chapters"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--book_name', type=str, help='The name of the book to generate an outline for')
