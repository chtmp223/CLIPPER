...
def format_data(row):
    ...
    if book in os.listdir("../../data/books/"):
        texts = [
            open(f"../../data/books/{book}/" + f, "r").read()
            for f in os.listdir(f"../../data/books/{book}/")
            if f.endswith(".txt")
        ]
    ...
    true_messages = [
        {
            "role": "system",
            "content": "You are an expert at verifying claims from fictional narratives.",
        },
        {"role": "user", "content": prompt_true},
    ]
    false_messages = [
        {
            "role": "system",
        ...
        {"role": "user", "content": prompt_false},
    ]
...
def process_text(text):
    try:
        ...
        if isinstance(text[1], dict) and "content" in text[1]:
            ...
            prompt = prompt.replace(" in at most one paragraph", "").replace(
                "</context>", "</context>\n"
            )
...
for i, book in tqdm(enumerate(book_names)):
    ...
    if os.path.exists(claims_path):
        if "claims_multiple.csv" in os.listdir(
            claims_path
        ) and "claims_single.csv" in os.listdir(claims_path):
    ...
    try:
        ...
        if "claims_multiple.csv" not in os.listdir(f"../../data/output/{book}/claims/"):
            ...
            if "claim_multiple_raw.csv" not in os.listdir(
                f"../../data/output/{book}/claims/"
            ):
        ...
        if "claims_single.csv" not in os.listdir(f"../../data/output/{book}/claims/"):
            ...
            if "claim_single_raw.csv" not in os.listdir(
                f"../../data/output/{book}/claims/"
            ):
...
for i, book in tqdm(enumerate(book_names)):
    ...
    if "claims_multiple.csv" not in os.listdir(f"../../data/output/{book}/claims/"):
        ...
        continue
    claims_multiple = pd.read_csv(
        f"../../data/output/{book}/claims/claims_multiple.csv"
    )
...
all_claims = (
    pd.concat(claims_df).drop_duplicates(subset=["facts"]).reset_index(drop=True)
)
all_claims["true_messages"], all_claims["false_messages"] = zip(
    *all_claims.apply(format_data, axis=1)
)
...
claims_final = pd.concat([claims_true, claims_false]).reset_index(drop=True)[
    ["book_name", "status", "messages", "claim_type", "facts", "corrupted_facts"]
]
claims_final["messages"] = claims_final["messages"].apply(process_text)

claims_final["id"] = range(0, len(claims_final))
claims_final = claims_final.drop_duplicates(
    subset=["book_name", "status", "claim_type", "facts", "corrupted_facts"]
