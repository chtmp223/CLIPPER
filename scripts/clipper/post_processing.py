...
def verify_claims_iterative(df, book_name): 
    ...
    outline = open(f"../../data/output/{book_name}/outline.md", "r").read().split("\n----------------\n")
    ...
    for i, claim in enumerate(tqdm(claims)): 
        ...
        all_source = [f"## Chapter {s+1} outline\n" + extract_tag_text(outline[s], "events", random=False).strip() for s in sources]
        source_text = "\n\n".join(all_source)
        prompt = p.format(book_summary=summary, claim=claim, chapter_outline=source_text)

        response = prompt_openai(prompt, verbose=False, metadata={"model": "gpt-4o-2024-08-06", "temp": 0.0,"top_p": 1.0,"max_tokens": 1000},system_message="You are an expert at verifying claims from fictional narratives.")
        try: 
...
def verify_claims(df, book_name): 
    ...
    for i, claim in enumerate(tqdm(claims)): 
        ...
        all_source = [f"## Chapter {s+1} outline\n" + extract_tag_text(outline[s], "events", random=False).strip() for s in sources]
        source_text = "\n\n".join(all_source)
        prompts.append(p.format(book_summary=summary, claim=claim, chapter_outline=source_text))
        
    df_response = format_batch_openai(prompts, "../../data/batch/verification_in.jsonl", "../../data/batch/verification_out.jsonl", metadata={"model": "gpt-4o-2024-08-06","max_tokens": 1000, "temp": 0.0, "top_p": 1.0, "system_message": "You are an expert at verifying claims from fictional narratives."})
...
def verify_corruption(df, book_name): 
    ...
    df_response = format_batch_openai(prompts, "../../data/batch/verify_corruption_in.jsonl", "../../data/batch/verify_corruption_out.jsonl", metadata={"model": "gpt-4o-2024-08-06","max_tokens": 1000, "temp": 0.0, "top_p": 1.0, "system_message": "You are an expert at verifying claims from fictional narratives."})
    responses = df_response.response.apply(extract_batch_content)
...
def duplicate_claims(claim_df): 
    ...
    prompt = open("../../prompts/duplication.md", "r").read().format(fact_list="\n".join(claim_list))
    retry = 0
    while retry < 2:
        try: 
            response = prompt_claude(prompt, verbose=False, metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0}, system_message="You are an expert at identifying duplicate claims.")

            # Extracting the answers
            if "<answer>" not in response and not re.search(r'\d', response): 
                claim_df['duplication'] = ["Unique" for i in range(len(all_claims))]
                claim_df['duplication_reasoning'] = ["Unique claim" for i in range(len(all_claims))]
            ...
            all_dups = []
            for a in answers: 
                dup_index = [int(item) for item in a.split(": ")[0].split(", ") if len(item) > 0 and item.isnumeric()]
            ...
            explanations = [re.sub(r'\- ', '', a) for a in explanations.split("\n") if len(a) > 0]
            all_exp = {}
            for a in explanations: 
                exp = a.split(": ")[1]
                claims = [item for item in a.split(": ")[0].split(", ") if len(item) > 0]
                for c in claims: 
                    all_exp[c] = exp
...
def post_processing_main(book_name, source, action_type): 
    ...
    for s in source:
        if action_type == "all": 
            ...
            df = df.drop_duplicates(subset=['facts', 'corrupted_facts']).reset_index(drop=True)
            df = duplicate_claims(df)
            df = df[df['duplication'] == "Unique"].reset_index(drop=True)
            df.to_csv(f"../../data/output/{book_name}/claims/claims_{s}_cleaned.csv", index=False)
            
            df = verify_claims_iterative(df, book_name)
            df = verify_corruption_iterative(df, book_name)
            df.to_csv(f"../../data/output/{book_name}/claims/claims_{s}_unfiltered.csv", index=False)
            df = df[(df['verify_facts'] == "VALID") & (df['verify_corruption']=="INVALID")].reset_index(drop=True)
            df.to_csv(f"../../data/output/{book_name}/claims/claims_{s}.csv", index=False)
            ...
            df = pd.read_csv(f"../../data/output/{book_name}/claims/claims_{s}_cleaned.csv")
            df = df.drop_duplicates(subset=['facts', 'corrupted_facts']).reset_index(drop=True)
            df = verify_claims_iterative(df, book_name)
            df = verify_corruption_iterative(df, book_name)
            df.to_csv(f"../../data/output/{book_name}/claims/claims_{s}_unfiltered.csv", index=False)
            df = df[(df['verify_facts'] == "VALID") & (df['verify_corruption']=="INVALID")].reset_index(drop=True)
            df.to_csv(f"../../data/output/{book_name}/claims/claims_{s}.csv", index=False)
        print(f"Extracted {len(df)} claims for {book_name} and saved to ../../data/output/{book_name}/claims_{s}.csv")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Extract claims from a story')
    parser.add_argument('--book_name', type=str, help='Name of the book to extract claims from')
    parser.add_argument('--source', help='single or multiple or both', default='both')
    parser.add_argument('--action_type', help='verify or duplicate or corrupt or all', default='all')
