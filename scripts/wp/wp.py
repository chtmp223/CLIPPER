...
def extraction(story, story_id): 
    ...
    responses = prompt_claude(
        prompt,
        verbose=True,
        metadata={'model': 'claude-3-5-sonnet@20240620', 'temp': 0.0, 'max_tokens': 4096, 'top_p': 1.0},
...
def wp(num_desired_claims):
    ...
    if not os.path.exists(output_path) or os.path.getsize(output_path) == 0:
        pd.DataFrame(columns=[
            "facts", "corrupted_facts", "fact_reasoning",
            "corrupted_reasoning", "source", "brainstorm", 
            "story", "id", "duplication", "duplication_reasoning"
    ...
    df_input = df_input[df_input['id'].astype(int) > already_processed].reset_index(drop=True)
    ...
    for i, story in enumerate(tqdm(stories), description="Extracting claims from stories"):
        ...
        prompt = open("../../prompts/duplication.md", "r").read().format(
            fact_list="\n".join(claim_list)
        )
        ...
        if "<answer>" not in response and not re.search(r'\d', response):
            ...
            df_full['duplication_reasoning'] = ["Unique claim" for _ in range(len(all_claims))]
            ...
            if len(answers) == 0:
                ...
                for a in answers:
                    ...
                    if len(split_line) < 2:
                        continue
                    try: 
                        duplicates_str, reasoning = split_line[0], " ".join(split_line[1:])
                    except: 
                        print(f"ERROR: {split_line}")
                        sys.exit()
                    # grab the indices before the colon
                    dup_index = [
                        int(item) for item in duplicates_str.split(", ") 
                        if item.isnumeric()
                    ]
                ...
                for a in answers:
                    ...
                    claims = [item for item in left_side.split(", ") if item.isnumeric()]
                    for c in claims:
                        explanations[c] = exp
                ...
                for idx in range(len(all_claims)):
                    if (idx+1) in dups_flat: 
                        ...
                        dup_exp.append(explanations.get(str(idx+1), "Likely duplicate"))
                    else: 
                ...
                print(f"Identified {len(dups_flat)} duplicate claims out of {len(all_claims)}.")
        
        df_full = df_full.drop_duplicates(subset=['facts', 'corrupted_facts']).reset_index(drop=True)
        df_full = df_full[~(df_full['duplication']=="Duplicate")].reset_index(drop=True)
        df_full.to_csv(
            output_path,
            index=False,
            header=False, 
