...
def get_summary(book_name):
    ...
    if (
        count_tiktoken(
            open(f"../../prompts/summary.md", "r").read().format(book=book_text),
            "gpt-4o",
        )
        > 124000
    ):
    ...
    summary = prompt_openai(
        summary_prompt,
        True,
        {"model": "gpt-4o-2024-08-06", "temp": 0.5, "top_p": 1.0, "max_tokens": 2000},
    )
...
def batch_summary():
    ...
    if "summary" in book_df.columns and type(book_df["summary"].tolist()[0]) == str:
        book_existing = book_df[
            (book_df["summary"].str.len() > 0) | (book_df["num_tokens"] > 128000)
        ]
    else:
    ...
    for book in tqdm(books):
        ...
        book_chapter = [
            ...
            for i, f in enumerate(os.listdir(f"../../data/books/{book}"))
        ]
        book_text = "\n\n".join(book_chapter)
    ...
    book_summary = format_batch_openai(
        summary_prompts,
        "../../data/batch/summary_input.jsonl",
        "../../data/batch/summary_output.jsonl",
        metadata={
            "model": "gpt-4o-2024-08-06",
            "max_tokens": 2000,
            "temp": 0.5,
            "top_p": 1.0,
            "system_message": "You are an expert at summarizing books.",
        },
    )
    ...
    print(
        len(book_df[book_df["summary"].str.len() == 0]),
        "books failed to generate summary",
    )
    return book_df
...
def main():
    ...
    parser.add_argument(
        "--book_name",
        type=str,
        help="Name of the book to summarize (if not using batch)",
        required=False,
    )
    parser.add_argument(
        "--batch",
        type=str,
        help="Batch generate summaries for all books (batch or nope)",
        default="nope",
    )
