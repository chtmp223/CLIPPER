...
def vllm_eval(
    df, claims, corrupted_claims, prompts, model_name="llama", model_dir=None
):
    """
        ...
        try:
            false_messages = tokenizer.apply_chat_template(
                [
                    {
                        "role": "user",
                        "content": prompts[i].format(claim=corrupted_claims[i]),
                    },
                ],
                tokenize=False,
                add_generation_prompt=True,
    ...
    for true_out in tqdm(true_output):
        try:
            ...
            output_answer = (
                extract_tag_text(output_reasoning, "answer", random=False)
                .strip()
                .lower()
                == "true"
            )
    ...
    for false_out in tqdm(false_output):
        try:
            ...
            output_answer = (
                extract_tag_text(false_out.outputs[0].text, "answer", random=False)
                .strip()
                .lower()
                == "false"
            )
...
def main():
    ...
    parser.add_argument(
        "--model", type=str, help="Models to evaluate", default="claude, gpt-4o, all"
    )
    parser.add_argument(
        "--model_dir", type=str, help="Model directory", default="claude, gpt-4o, all"
    )
    parser.add_argument("--data_input", type=str, help="Input data path", required=True)
    parser.add_argument(
        "--data_output", type=str, help="Output data path", required=True
    )
    parser.add_argument(
        "--subset", type=str, help="gutenberg or 2024", default="gutenberg"
    )
    parser.add_argument("--start_row", type=int, help="Start row index")
    parser.add_argument("--end_row", type=int, help="End row index")
    parser.add_argument(
        "--chunking", type=str, help="Whether to chunk or not", required=True
    )
    ...
    claims_df_batch = (
        pd.read_csv(data_input)
        .drop_duplicates(subset=["facts", "corrupted_facts"])
    ...
    if os.path.exists(data_output):
        ...
        if existing_facts != input_facts:
            print(
                "Mismatch between existing facts and input data. Running the full data."
            )
        else:
    ...
    if args.subset == "nocha" and args.model in ["llama", "qwen"]:
        claims_df_batch = claims_df_batch[claims_df_batch.length <= 128000].reset_index(
            drop=True
        )
    ...
    for i, book in enumerate(claims_df_batch[col].tolist()):
        ...
        if args.subset == "gutenberg":
            if book in os.listdir(f"../../data/books/"):
                all_chaps = sorted(
                    os.listdir(f"../../data/books/{book}/"),
                    key=lambda x: int(x.split(".")[0]),
                )
                texts = [
                    open(f"../../data/books/{book}/" + f, "r").read() for f in all_chaps
                ]
            else:
    ...
    if args.model.lower() not in ["gpt-4o", "o1-mini"]:
        ...
        print(
            "Overall accuracy:",
            len(
                df[(df.answer_true == True) & (df.answer_false == True)].reset_index(
                    drop=True
                )
            )
            / len(df)
        ...
        print(
            ...
            len(
                df[
                    (df.answer_true == "Error") | (df.answer_false == "Error")
                ].reset_index(drop=True)
            ),
        )
